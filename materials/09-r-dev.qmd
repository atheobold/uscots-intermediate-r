...
Moving from "scripts that work" to "professional code":
...
4. **Have resources to teach professional development practices**
:::

. . .

::: {style="font-size: 1.1em; color: #2E7D32;"}
**Focus: Teaching development mindset and multilingual data science**
:::
...
microbenchmark(
  ...
  data.table_approach = penguins[, .(mean_mass = mean(body_mass_g, na.rm = TRUE)), 
                                by = species],
  times = 100
)
```
...
**Teaching Value**: Students learn to compare approaches objectively with real data
:::
...
analyze_penguin_measurements <- function(data, measurement_var) {
  data %>%
...
bill_analysis <- analyze_penguin_measurements(penguins, bill_length_mm)
mass_analysis <- analyze_penguin_measurements(penguins, body_mass_g)
```
...
::: {.incremental}
- **Best tool for the job**: Use R for statistics, Python for ML
- **Team collaboration**: Different team members prefer different languages
- **Ecosystem access**: Leverage both CRAN and PyPI packages
- **Career preparation**: Industry expects multilingual skills
:::
...
# R: Business logic and reporting
create_penguin_report <- function(data_source) {
  # R handles data loading and initial processing
  raw_data <- read_data(data_source)
  
  # Pass to Python for advanced analytics
  py$raw_penguins <- raw_data
  py_run_string("
    import pandas as pd
    from sklearn.preprocessing import StandardScaler
    
    # Advanced preprocessing in Python
    scaler = StandardScaler()
    scaled_features = scaler.fit_transform(raw_penguins.select_dtypes(include=[np.number]))
    processed_data = raw_penguins.copy()
  ")
  
  # Bring results back to R for reporting
  processed_data <- py$processed_data
  
  # R handles visualization and reporting
  create_final_report(processed_data)
}
```
...
- Start with simple data passing between languages
- Emphasize when to use each language's strengths
- Show collaborative workflow examples
- Practice debugging across language boundaries
:::
...
1. **Week 1**: Performance awareness with microbenchmark
2. **Week 2**: Code readability principles and refactoring
3. **Week 3**: Systematic profiling with profvis
4. **Week 4**: Basic R-Python data exchange
5. **Week 5**: Advanced multilingual workflows

**Assessment Strategies:**
- Performance improvement portfolios
- Code review exercises focusing on readability
- Multilingual project implementations
- Debugging exercises across languages
:::
...
## Activity: Design Your Developer Curriculum

::: {.midi}
**In small groups (15 minutes):**

1. **Choose your focus**: Performance, readability, or multilingual skills?
2. **Design progression**: How do concepts build on each other?
3. **Select assessment methods**: How will you evaluate developer thinking?
4. **Plan integration**: How does this connect to domain-specific content?
5. **Consider constraints**: What are your students' programming backgrounds?

**Be ready to share your approach and discuss challenges!**
:::